...
def hexof(c: dict) -> str:
    """iTerm2 color dict -> 'RRGGBB'. The inverse of color(), so the shell
    palette emitter reads the same built profiles the JSON does — one source."""
    return "".join(
        f"{round(c[k] * 255):02X}"
...
# ---------------------------------------------------------------------------
# Shell palette emitter  --  the same palettes as a zsh-sourceable data file,
# so `retro random` (retro-prompts.zsh) can repaint the live terminal via OSC
# escapes. Keys match the _RETRO_PROMPTS keys in retro-prompts.zsh so one key
# selects palette + prompt + banner together. Machine name -> short key:
# ---------------------------------------------------------------------------
SHELL_KEY = {
    "Paper Teletype": "teletype", "CRT Green Phosphor": "green",
    "CRT Amber Phosphor": "amber", "Linux Console": "linux",
    "SunOS cmdtool": "sun", "IRIX wsh": "irix", "Commodore 64": "c64",
    "MS-DOS Editor": "dos", "Amiga Workbench": "amiga", "NeXTSTEP": "next",
    "OpenStep": "openstep", "Apple II": "apple2", "ZX Spectrum": "spectrum",
    "Atari 8-bit": "atari", "BBC Micro Mode 7": "bbc", "Plan 9 acme": "plan9",
    "The Matrix": "matrix", "TRON": "tron", "Neuromancer": "neuromancer",
    "HAL 9000": "hal", "Fallout Pip-Boy": "pipboy", "Severance Lumon": "severance",
    "Cyberpunk 2077": "cyberpunk", "Blade Runner": "bladerunner", "WOPR": "wopr",
...
def _era_by_name():
    """Pull each machine's `era` label out of the gallery (index.html), which is
    its single source. Returns {machine name: era}. Empty if index.html absent —
    the era line is a nicety, never load-bearing."""
    import re
...
def write_shell_palettes(dest_dir):
    """Write retro-palettes.zsh: per-machine bg/fg/cursor/16-ansi hexes, name and
    era, keyed by short name, plus an ordered _RETRO_ALL list for the random pick."""
    lines = [
    ...
    for p in PROFILES + FICTION + AESTHETIC + CORP:
        ...
        if not key:
            ...
            continue
        ansi = " ".join(hexof(p[f"Ansi {i} Color"]) for i in range(16))
        lines.append(
            f"_RETRO_BG[{key}]={hexof(p['Background Color'])}; "
            f"_RETRO_FG[{key}]={hexof(p['Foreground Color'])}; "
            f"_RETRO_CUR[{key}]={hexof(p['Cursor Color'])}; "
            f"_RETRO_ANSI[{key}]='{ansi}'"
        )
        lines.append(
            f"_RETRO_NAME[{key}]={_sq(p['Name'])}; "
            f"_RETRO_ERA[{key}]={_sq(era.get(name, ''))}"
        )
        keys.append(key)
    lines.append("")
    lines.append("_RETRO_ALL=(" + " ".join(keys) + ")")
    lines.append("")
    dest = os.path.join(dest_dir, "retro-palettes.zsh")
    with open(dest, "w") as f:
        f.write("\n".join(lines))
    print(f"Wrote {len(keys):>2} palettes    -> retro-palettes.zsh")
    if unmapped:
        # Telemetry: a new machine without a SHELL_KEY entry is invisible to
        # `retro random`. Surface it loudly rather than dropping it silently.
        print(f"    WARNING: {len(unmapped)} profile(s) missing a SHELL_KEY: "
              f"{', '.join(unmapped)}")
